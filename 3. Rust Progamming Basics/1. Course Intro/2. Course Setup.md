...
For instance, the `REAMDE.md` in `topics/scalar/exercises/` might contain an instruction like:

> "Exercise 1: Fix the function `eq` insdie `src/lib.rs`. Compare 2 inputs of the type `char` for equality and return a `bool`."
...
The `README.md` within each topic's `exercises` folder (e.g., `topics/scalar/exercises/README.md`) will provide the necessary command to run any associated examples. Look for a section title "Example" or similar.
...
This comman instructs Cargo to compile and execute the specified example file. The `--release` flag compiles the code with optimizations.
